...
Pour commencer, chargeons quelques packages: le **tidyverse**, **broom**, qui permet de gérer les estimations de modèle comme des tibbles, **DT**, pour présenter les tableau sous forme d'applet html, **plm**, qui permet d'estimer les modèles de panel classiques, et **panelr**, qui permet notamment d'estimer les modèles hybrides.
...
datatable(g_i$coefficients, options = list(pageLength = 10)) %>% 
   formatRound(columns=c('(Intercept)', 'wks'),digits=8)
```
...
datatable(dum_mod%>% tidy(), options = list(pageLength = 10)) %>% 
   formatRound(columns=c('estimate', 'std.error','statistic','p.value'),
               digits=8)
```
