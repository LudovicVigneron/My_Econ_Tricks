...
**Bon! J'aurais peut-être dû commencer par là. Les données de panel présentent une structure particulière la répétition des observations dans le temps permet d'appréhender à la fois des différences entre individus et entre périodes mais aussi pour les individus sur les périodes ou pour les périodes en fonction des individus... Cela se concrétise par des tendances au niveau de données plus précisément au niveau de la variation de leur valeurs des variables. C'est autour de à l'appréhension de cette variabilité, à sa mesure, que ce court billet s'organise.**
...
Revenons sur nos données de base et intéressons-nous à la variabilité de notre variable expliquée (lwage). Cette variabilité est généralement exprimée par la variance ($s^2$) ou l'écart type ($s$) (construit à partir de somme des écarts carrés à la moyenne - sec). Recalculons-les.

```{r}
wages %>% select(lwage) %>% 
  summarise_all(c(var,sd,function(x){var(x)*(n()-1)})) %>% 
  round(digits=5) %>% 
  rename(variance=fn1,ecart_type=fn2,somme_écarts_carrés=fn3)
```

Dans le cadre de données de panel, ces indicateurs est leur sous-jacent sont qualifiés de globaux (Overall). La variance (échantillon) globale correspond à la variance classique.
...
La plus courante consiste à distinguer la variabilité associée aux différences entre individus (Between individuals) indépendamment du temps (variabilité inter-individuelle) et la variabilité associée à l'évolution de chaque individu dans le temps (Within individuals) indépendamment de ses caractéristiques invariable (variabilité intra-individuelle). Ces deux sources de variabilité se cumulent pour former la variabilité globale. On a ainsi:
...
On retrouve souvent cette valeur présentée sous forme de la variance inter-individuelle (Between variance ou Between individual variance). Elle est juste normé par le nombre d'individus moins (puisque l'on est sur une variance échantillon).
...
La variabilité intra-individuelle ($sec_{W_i}$) se mesure par la somme des écarts carrés valeurs individuelles aux moyennes individuelles sur l'ensemble de la période.
...
Cette valeur peut être présentée sous forme de la variance inter-temporelle (Between variance ou Between time variance). Elle est juste normé par le nombre de périodes moins une (puisque l'on est sur une variance échantillon).
...
Encore une fois la variabilité intra-temporelle est plus généralement présenté via la variance associée dont l'expression est la suivante:
...
La variance globale peut être retrouvé en ajustant la somme des variances between time et within time.
...
En plus de la variabilité d'une série de statistiques descriptives classiques on nous donne la variabilité globale (somme des carrés des écarts) ainsi qu'une décomposition id time correspondant à la part de la variabilité between de ces dimensions dans la variabilité totale.
...
L'ensemble peut être avec un peu de travail étendue et synthétisé dans un tableau unique. Commençons par créer des fonctions nous permettant de sortir les variations within et between.
...
L'analyse peut être appliqué à notre variable explicative (wks).
