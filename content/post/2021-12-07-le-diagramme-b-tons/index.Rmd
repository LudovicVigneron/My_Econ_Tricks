...
```{r}
draft_1<-df_drafts %>% filter(numberPickOverall==1)
draft_1
```
...
ggplot(draft_1, aes(nameTeam)) +
  ...
  labs(title='1er choix de draft NBA (1983-2018)',
       caption = 'Basketball Reference (via nbastatR)')+
  coord_flip() +
  theme_minimal() +
  theme(plot.title=element_text(face='bold',hjust=0.5),
        plot.caption=element_text(face='italic'),
        axis.title = element_blank(),
        axis.text.y = element_text(margin = margin(l = 20, r = -140),
        vjust=-0.5,hjust=0),
        panel.grid.major.x = element_blank(),
        panel.grid.minor.x = element_blank(),
        panel.grid.major.y = element_blank()) -> gra_draf1
gra_draf1
```
